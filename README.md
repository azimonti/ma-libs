...
- C++ neural network library with BLAS/LAPACK backend and HDF5 for storing the weights so that can be later reused (built with `--cmake-params "-DCPP_LIBNN=ON"` or `--build-suite`). It can be optionally be build with BLAS/LAPACK (built with `--cmake-params "-DCPP_LIBNN_BLAS=ON"`) or BLAS/LAPACK and HDF5 (built with `--cmake-params "-CPP_LIBNN_BLAS_HDF5=ON"`). Optionally Python bindings can be created with the optional arguement `PYTHON_BINDINGS=ON` (so `--cmake-params "-DCPP_LIBNN=ON -DPYTHON_BINDING=ON"`). A Python virtual environment can be created if needed with the script `cpp/libnn/create_virtualenv.sh`.
